...
public class AuthService {
	...
	public AuthService(JWTTokenRepository jwtTokenRepository, UserRepository userRepository, @Value("${jwt.secret}") String jwtSecret  ) {
		...
		if(jwtSecret.getBytes(StandardCharsets.UTF_8).length < 64) {
			throw new IllegalArgumentException("JWT_SECRET in application.");
		}
		
		this.SIGNING_KEY = Keys.hmacShaKeyFor(jwtSecret.getBytes(StandardCharsets.UTF_8)); 		// here generating the keys
	}
	
	public User authenticate(String username, String password) {
		User user = userRepository.findByUsername(username).orElseThrow(() -> new RuntimeException("Invalid username and password"));
